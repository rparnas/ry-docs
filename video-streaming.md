...
This method is useful if you have enough bandwidth to directly host a stream to your participants.
...
* On your router, add the following port-forwarding rules. Also add these as *Inbound* rules in the Windows Firewall.
  ...
  * SRS-WebRTC-UDB (TCP, port 8000)
  * SRS (TCP, port 8080)
* SRS-WHEP is for control/signalling, SRS-WebRTC-UDP is for the audio/video stream, SRS is for the built-in web server.
* You may additionall forward SRS-RTMP (TCP, port 1935) if you to stream OBS from an external machine to SRS.
